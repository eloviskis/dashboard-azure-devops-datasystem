...
import { WorkItem, WorkItemStatus, WorkItemFilters } from '../types.ts';
...
const MonteCarloSimulation: React.FC<MonteCarloSimulationProps> = ({ data, filters }) => {
    ...
    const selectedSquadsText = useMemo(() => {
        if (filters.squads.length > 0) {
            return filters.squads.join(', ');
        }
        return 'todas as squads';
    }, [filters.squads]);
            ...
            <div className="bg-ds-navy p-6 rounded-lg border border-ds-border">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
                 ...
                 <div className="text-xs text-ds-muted mt-4 space-y-1">
                    <p><strong className="text-ds-text">Contexto da Análise:</strong> {selectedSquadsText}</p>
