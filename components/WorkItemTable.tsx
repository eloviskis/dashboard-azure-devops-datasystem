...
type SortKey = keyof WorkItem | 'cycleTime';
    ...
    if (sortConfig !== null) {
      sortableItems.sort((a, b) => {
        ...
        if (sortConfig.key === 'cycleTime') {
            aValue = a.performanceDays;
            bValue = b.performanceDays;
        } else {
            aValue = a[sortConfig.key as keyof WorkItem];
            bValue = b[sortConfig.key as keyof WorkItem];
        }
        ...
        <table className="w-full text-sm text-left text-ds-text">
            <thead className="text-xs text-ds-light-text uppercase bg-ds-navy/50">
                <tr>
                    ...
                    <SortableHeader sortKey="assignee" title="Responsável" sortConfig={sortConfig} requestSort={requestSort} />
                    <SortableHeader sortKey="squad" title="Squad" sortConfig={sortConfig} requestSort={requestSort} />
            ...
            <tbody>
                {paginatedItems.map((item) => (
                    <tr key={item.id} className="bg-ds-navy border-b border-ds-border hover:bg-ds-muted/20">
                        <td className="px-6 py-4 font-medium text-ds-light-text whitespace-nowrap">{item.id}</td>
                        <td className="px-6 py-4 max-w-xs truncate" title={item.title}>{item.title}</td>
                        <td className="px-6 py-4">
                            <span className="flex items-center">
                                <span className="h-2 w-2 rounded-full mr-2" style={{ backgroundColor: STATUS_COLORS[item.status] }}></span>
                                {item.status}
                            </span>
                        </td>
                        <td className="px-6 py-4">{item.assignee}</td>
                        <td className="px-6 py-4">{item.squad}</td>
                        <td className="px-6 py-4">{item.type}</td>
                        <td className="px-6 py-4">{format(item.createdDate, 'dd/MM/yyyy')}</td>
                        <td className="px-6 py-4 text-center">{item.status === WorkItemStatus.Concluido ? item.performanceDays : '-'}</td>
                        <td className="px-6 py-4">
                            {item.tags.map(tag => (
                                <span key={tag} className="inline-block bg-ds-muted/50 text-ds-light-text text-xs font-semibold mr-2 mb-1 px-2.5 py-0.5 rounded-full whitespace-nowrap">
                                    {tag}
                                </span>
                            ))}
                        </td>
