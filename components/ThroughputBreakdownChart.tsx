...
interface ThroughputBreakdownChartProps {
  data: WorkItem[];
  groupBy: 'assignee' | 'type';
}

const ThroughputBreakdownChart: React.FC<ThroughputBreakdownChartProps> = ({ data, groupBy }) => {
  const chartData = useMemo(() => {
    const completed = data.filter(item => item.status === WorkItemStatus.Concluido && item[groupBy]);
  ...
  if (chartData.length === 0) {
    return <EmptyState message={`Nenhum item concluído para detalhar a vazão por ${groupBy === 'assignee' ? 'responsável' : 'tipo'}.`} />;
  }
  ...
  return (
    <ResponsiveContainer width="100%" height={350}>
      <BarChart data={chartData} layout="vertical" margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
        ...
        />
        <Bar dataKey="value" name="Itens Concluídos" fill={groupBy === 'assignee' ? CHART_COLORS.primary : CHART_COLORS.secondary} />
      </BarChart>
    </ResponsiveContainer>
  );
};
