...
const CalendarImportMulti: React.FC<CalendarImportMultiProps> = ({ teams, month, onClose, onImported }) => {
  ...
  const handleUrlFetch = async () => {
    if (!icsUrl.trim()) { alert('Cole a URL do calendário'); return; }
    setEvents([]);
    setSelected(new Set());
    setLoading(true);
    try {
          ...
          {events.length === 0 && (
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-ds-muted text-xs mt-1">Outlook: Arquivo → Salvar calendário → .ics</p>
              </div>
              ...
              {/* URL compartilhada */}
              <div>
                <label className="text-ds-text text-xs font-semibold block mb-2">🔗 Link do calendário compartilhado (Outlook/Exchange)</label>
                ...
                <p className="text-ds-muted text-xs mt-1">💡 Outlook → Configurações → Calendário → Calendários compartilhados → Publicar → Copiar link ICS</p>
              </div>
              ...
              {!loading && (file || icsUrl) && events.length === 0 && (
                <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-3 text-yellow-400 text-sm">
                  ...
                  <p className="text-xs mt-1">Buscando por: refinamento, review, retrospectiva, daily, planning, apresentação, resultado (01/06/2025 até hoje)</p>
                </div>
              )}
            </div>
          )}
