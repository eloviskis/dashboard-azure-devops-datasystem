...
const TopReviewersChart: React.FC<TopReviewersChartProps> = ({ data }) => {
  ...
  const handleClick = (entry: any) => {
    const items = data.filter(pr => 
      pr.reviewers.some((r: any) => r.name === entry.name)
    );
    setModalData({
      title: `PRs revisados por ${entry.name}`,
      items,
      ...
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={chartData} layout="vertical" margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
          ...
          <Tooltip
            ...
            formatter={(value: number) => [value, 'Revisões de PR (clique para ver)']}
          />
          <Legend />
          <Bar dataKey="reviews" name="Revisões de PR" fill={CHART_COLORS.secondary} cursor="pointer">
