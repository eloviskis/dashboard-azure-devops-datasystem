...
const BugVsIssuePieChart: React.FC<BugVsIssuePieChartProps> = ({ data }) => {
  ...
  const chartData = useMemo(() => {
    ...
    return [
      ...
      { name: 'Issues', value: counts.issues },
    ].filter(item => item.value > 0);
  }, [data]);

  const handleClick = (entry: any) => {
    const items = data.filter(item => {
      if (entry.name === 'Bugs') return item.type === 'Bug';
      if (entry.name === 'Issues') return item.type === 'Issue';
      return false;
    });
    const colorIndex = entry.name === 'Bugs' ? 0 : 1;
    setModalData({
      title: `${entry.name}`,
      items,
  ...
  if (chartData.length === 0) {
    return <EmptyState message="Nenhum bug ou issue para exibir." />;
  }
