      ...
      <div className="bg-ds-navy p-4 rounded-lg border border-ds-border">
        ...
        <ResponsiveContainer width="100%" height={Math.max(250, analysis.teamData.length * 45)}>
          <BarChart data={analysis.teamData} layout="vertical" margin={{ left: 120, right: 70 }}>
            ...
            <Legend />
            <Bar dataKey="activeTime" name="Tempo Ativo (dias)" stackId="a" fill="#64FFDA" radius={[0, 0, 0, 0]} label={{ position: 'inside', fill: '#0a192f', fontSize: 10, fontWeight: 'bold', formatter: (value: any, entry: any) => entry?.activeTime?.toFixed(1) || value.toFixed(1) }} />
            <Bar dataKey="waitTime" name="Tempo em Espera (dias)" stackId="a" fill="#F56565" fillOpacity={0.6} radius={[0, 4, 4, 0]} label={{ position: 'inside', fill: '#fff', fontSize: 10, fontWeight: 'bold', formatter: (value: any, entry: any) => entry?.waitTime?.toFixed(1) || value.toFixed(1) }} />
          </BarChart>
        </ResponsiveContainer>
